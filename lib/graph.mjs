...
// Vendored fflate UMD bundle (lib/vendor/fflate-umd.js) rather than an npm
// dependency: the generated HTML needs fflate as a plain-script `fflate`
// global (no module system in the classic <script> that runs before the
// deferred Svelte bundle), and vendoring keeps the published package — and
// the GitHub Action, which runs from a checkout with no node_modules —
// fully self-contained with zero runtime npm dependencies.
// Vendored from fflate@0.8.3 (MIT, https://github.com/101arrowz/fflate).
const FFLATE_UMD = path.join(path.dirname(fileURLToPath(import.meta.url)), 'vendor', 'fflate-umd.js');
...
export function extractGraph(projectRoot, db, sqliteJson, log = () => {}, gitDiffRef = null) {
  ...
  // Decorator-based routing (NestJS, FastAPI, Spring-style annotations, ...)
  // puts everything CodeGraph doesn't structurally capture — `@ApiResponse`,
  // `@ApiBody`, `@HttpCode`, `@Public`, `@UseGuards` — on the lines directly
  // above the declaration, outside the symbol's own start/end range. Widens
  // the snippet's start line to swallow that contiguous decorator block, so
  // downstream regex scanning (auth/status/response-type detection) can see
  // it. Walks upward while at bracket depth 0 the line begins a new `@Decorator`
  // (bracket depth lets a single decorator's args span several lines without
  // ending the walk early); stops at the first line that doesn't fit that
  // shape, or after 30 lines as a safety cap.
  function widenForDecorators(lines, startLine) {
    let i = startLine - 2; // 0-indexed line just above startLine
    let depth = 0;
    let widenedStart = startLine;
    const floor = Math.max(0, startLine - 41);
    while (i >= floor) {
      const line = lines[i];
      const trimmed = line.trim();
      if (depth === 0 && trimmed === '') break; // blank line at top level ends the block
      // This line's own bracket balance, scanned right-to-left (walking
      // upward): a lone `)` means we're inside a construct whose opener is
      // further up still: only meaningful once combined with `depth`.
      let delta = 0;
      ...
      // At top level (depth 0 both before and after this line), the line
      // must itself open a new decorator — anything else means we've
      // walked past the decorator block into unrelated code above it.
      if (newDepth === 0 && depth === 0 &&
          !trimmed.startsWith('@') && !trimmed.startsWith('//') && !trimmed.startsWith('*') && !trimmed.startsWith('/*')) {
        break;
      }
      depth = newDepth;
      widenedStart = i + 1; // 1-indexed
      i--;
    }
    return widenedStart;
  }
  function readSnippet(filePath, startLine, endLine) {
    ...
    if (!lines) return '';
    const from = Math.max(1, widenForDecorators(lines, startLine));
  ...
  return {
    ...
    // The git ref the diff was taken against (null when no --diff was given),
    // so the viewer can label "changed since <ref>" instead of a bare flag.
    changedRef: gitDiffRef || null,
